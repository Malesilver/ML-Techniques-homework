...
def adaboost_stump(data_x, data_y, T=300):
    ...
    sum_weight_list = np.zeros(T)
    epsilon_list = np.zeros(T)
    ...
    for k in range(T):
        ...
        if epsilon == 0:
            ...
            sys.exit(-1)
        # save weight
        sum_weight_list[k] = np.sum(weight)
        # updata weight
        ...
        i_list[k] = i
        ## save epsilon
        epsilon_list[k] = epsilon
    return sum_weight_list, epsilon_list


def pred_sign(alpha_list,s_list,theta_list,i_list,data):
    T = alpha_list.shape[0]
    ...
    for k in range(T):
        ...
        theta = theta_list[k]
        i = int(i_list[k])
        sign_pred = s * get_sign(data[:, i] - theta)
        sum = sum + sign_pred * alpha
    res = get_sign(sum)
    return res
...
if __name__ == '__main__':
    ...
    T = 300
    sum_weight_list, epsilon_list = adaboost_stump(data_x, data_y, T=T)

    plot_line_chart(Y=sum_weight_list, nameY="Ut", saveName="15.png")
    print("U2:",sum_weight_list[1])
    print("UT:",sum_weight_list[-1])

    plot_line_chart(Y=epsilon_list,nameY="epsilon_t",saveName="16.png")
    print("min value of epsilon_t:",min(epsilon_list))
    '''
    U2: 0.854166260163
    UT: 0.00546512411935
    min value of epsilon_t: 0.178728070175
    '''
