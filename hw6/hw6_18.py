...
def pred_sign(t, alpha_list, s_list, theta_list, i_list, data):
    ...
    for k in range(T):
        ...
        if t == 0:
            ...
            sum = sum + np.multiply(sign_pred, alpha)
    res = get_sign(sum)
    return res
...
if __name__ == '__main__':
    ...
    for n in range(T):
        sign_pred_test = \
            pred_sign(n, alpha_list, s_list, theta_list, i_list, test_x)
        err_train = get_err(test_y, sign_pred_test)
        err_test_list[n] = err_train

    plot_line_chart(Y=err_test_list,nameY="Eout(Gt)",saveName="18.png")
    print('Eout(G):', err_test_list[-1])
