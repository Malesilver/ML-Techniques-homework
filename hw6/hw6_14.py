...
def pred_sign(t, alpha_list, s_list, theta_list, i_list, data):
    alpha_list = alpha_list[:t+1]
    s_list = s_list[:t+1]
    theta_list = theta_list[:t+1]
    i_list = i_list[:t+1]
...
if __name__ == '__main__':
    ...
    T = 300
    alpha_list, s_list, theta_list, i_list = adaboost_stump(data_x, data_y, T=300)

    err_train_list = np.zeros(T)
    for n in range(T):
        sign_pred_train = \
            pred_sign(n, alpha_list, s_list, theta_list, i_list, data_x)
        err_train = get_err(data_y, sign_pred_train)
        err_train_list[n] = err_train

    plot_line_chart(Y=err_train_list,nameY="Ein(Gt)",saveName="14.png")
    print('Ein(G):',err_train_list[-1])
