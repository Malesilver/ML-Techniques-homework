...
class One_branch_Tree(object):
    ...
    def build_tree(self,data_x,data_y):
        if np.abs(np.sum(data_y)) == data_y.shape[0]:
            ...
            c1_index = np.array(c1_index)
            c2_index = np.array(c2_index)
            sign_c1 = data_y[c1_index]          # 数据1；划分标记为1
            sign_c2 = data_y[c2_index]          # 数据2：划分标记为-1
            val_L = 1 if np.sum(sign_c1) > 0 else -1
            val_R = 1 if np.sum(sign_c2) > 0 else -1
            node.Lchild = Node((0, val_L, 0))
            node.Rchild = Node((0, val_R, 0))
        self.root = node

    def get_val(self, x):
        node = self.root
        if node.val[0] != 0:
            s, theta, i = node.val[0], node.val[1], node.val[2]
...
class RandomForest(object):
    ...
    def build_RF(self, data_x, data_y):
        for i in range(self.n_tree):
            ...
            dec_tree = One_branch_Tree()
            dec_tree.build_tree(sample_x, sample_y)
            self.trees.append(dec_tree)

    def predict(self, data_x):
        y_predict = np.zeros((self.n_tree, data_x.shape[0]))
        for i in range(self.n_tree):
            dec_tree = self.trees[i]
            y_predict[i] = dec_tree.predict(data_x)
...
def plot_line_chart(X=np.arange(0,3000,1),Y=np.arange(0,300,1),
                    nameX="t",nameY="Ein(Gt)",saveName="19.png"):
    X = list(X)
    Y = list(Y)
...
if __name__ == "__main__":
    ...
    plot_line_chart(Y=Ein_Gt, nameY="Ein(Gt)", saveName="19.png")
    ...
    plot_line_chart(Y=Eout_Gt, nameY="Eout(Gt)", saveName="20.png")
